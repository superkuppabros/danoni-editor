...
export class ScoreRevivalService {
  ...
  dosConvert(dosData: string): ScoreData {
    const splitArray = _.compact(dosData.replace(/\n/g, "").split(this.delimiter))
    const dict: {[name: string]: string} =  _.fromPairs(splitArray.map(elem => elem.split("=")))
    try {
  ...
  private calcScoreData = (dict: {[name: string]: string}, timings: Timing[], scoreData: ScoreData) => {
    ...
    const [noteFrames, freezeFrames, speedChangeFrames]: (number)[][][] = [noteNames, freezeNames, speedChangeNames]
      .map(names => names.map(
        (keyNameWithNum: string) => keyNameWithNum.replace(scoreNumber.toString(), ""))
        .map((keyName: string) => dict[keyName].split(",").filter(x => x).map((x: string) => Number(x))))
